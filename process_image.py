...
def process_upscale(img):
    print("[+] Menjalankan Lanczos4 Resampling + Unsharp Masking (2x Scale)...")
    h, w = img.shape[:2]
    
    # Langkah 1: Upscale menggunakan Lanczos8/4 interpolasi (paling tajam di kelasnya)
    upscaled = cv2.resize(img, (w * 2, h * 2), interpolation=cv2.INTER_LANCZOS4)
    
    # Langkah 2: Buat Unsharp Mask untuk mempertegas sudut & detail agar tidak burik
    gaussian_blur = cv2.GaussianBlur(upscaled, (0, 0), 2.0)
    # Rumus pencampuran pixel untuk mengembalikan micro-contrast tepi objek
    sharpened = cv2.addWeighted(upscaled, 1.6, gaussian_blur, -0.6, 0)
